...
The resulting `.handoffs`, `.broll-plans`, `.broll-selections`, and `.broll-motions` folders are adjacent to the project JSON and must be backed up together. They reference catalog image IDs rather than fixed artwork paths. After motion planning, **Review B-roll decisions** opens a scrollable image-and-motion review. Clicking a candidate thumbnail saves its choice immediately; motion and speed controls also save on change. Click inside a detected face/object box to use it as the motion anchor, or use **Center anchor**. **Merge ↑/↓** folds the clicked beat into its neighbor: the neighbor keeps its image, both passages and candidate lists combine, and motion is recalculated over the full interval without changing locked word times. Detections, anchor, and crop boxes appear on the centered full-frame image; the green zoom-in final box or red zoom-out initial box shows the actual tighter crop. A red outline around the whole preview flags preferred safety limits without blocking a manual choice. The review also shows context, coverage warnings, and an XML track preflight. **Export B-roll Premiere XML** creates direct, editable still clips with sparse upper tracks and native motion keyframes while retaining the phase-1 talking-head and audio clips. The revised Electron review and export controls and Premiere import have not yet been interactively verified against a production project; treat XML output as a testable candidate, not an accepted production interchange.

Each review beat also has **AI Search**. Its modal starts with that beat's saved visual query and available tag assumptions; edit the description, expand **Search tags** to change Book, Character, Setting, Mood, or Image Type filters, and set **Results** from 1 to 50 before searching the local hybrid index. This does not call the hosted model. **Use this image** saves one sparse override, even when the image was not in the original eight candidates. The chosen image must still meet that beat plan's saved artwork minimum. Accepted artwork paths are resolved from the current catalog rather than stored in the project. The pencil on any displayed candidate opens its accepted-tag editor, including **Deactivate this image in the catalog**. Deactivation excludes it from future results and makes a currently selected image unavailable for export without deleting the catalog record. After changing tags, manual AI Search can continue over images still indexed; it reports how many edited images are temporarily omitted. Use **Tag → Update Embeddings** when ready to include them again. An unchanged image file can remain selected in the meantime.
...
Choosing a different image in B-Roll review saves that beat immediately and calculates the new image's crop and scale. For rate changes, **Save motion settings** affects future plans only. **Apply to current motion · no AI** in configuration—or **Recalculate crops · no AI** in the B-Roll toolbar—updates the current base motion and manual override crops procedurally. If nothing changed, it reports that motion is already current. Reopen B-Roll review to inspect changed crops before exporting. Tag or schema edits never require rerunning beat planning for this step. **AI plan motion** is a separate hosted-model action, not the refresh control.
