...
- A separate motion action and configurable rate/safety settings produce a fingerprinted adjacent `.broll-motions` artifact, which also travels with the project. A hosted text agent chooses direction, speed, and a local detection anchor (or center); deterministic geometry computes centered fill-frame zoom endpoints and pan crops. Infeasible or subject-clipping motion falls back to static with warnings, and magnification can flag poor effective detail. Geometry and provider interaction are unit-tested, not verified in a live project or Premiere.
- M6 has a combined scrollable review UI backed by the accepted-image catalog. It shows each beat's text and blue bookending context, candidate filenames and availability, detection/anchor overlays, fill-frame crop and keyframe boxes, and coverage warnings. Candidate, motion direction, speed, and anchor edits append one sparse layer for exactly one locked beat range in the project JSON; older layers and unrelated beats remain intact. The review resolves current catalog paths on demand and checks artwork existence before preview/save. Its state/geometry and persistence are tested, but the Electron UI has not been interactively verified.
