...
- The motion settings now have a separate **Apply settings to current motion (no AI)** action. It saves the form values, deterministically recalculates all current base crops into a new fingerprinted motion artifact, and appends recalculated copies of active sparse manual override layers under the new motion ID. Original plans, older/historical override layers, image choices, directions, speeds, anchors, and locked word timing remain untouched. The button requires a current motion plan; merely saving settings still affects future plans only. Core tests and a read-only Jadis-plan recalculation passed; the new UI action has not yet been interactively verified.
- M6 has a combined scrollable review UI backed by the accepted-image catalog. It shows each beat's text and blue bookending context, one centered fill-frame preview with detection, anchor, and keyframe overlays, plus clickable thumbnails for the other candidate images. The image stays at the full-frame crop while the zoom-in final or zoom-out initial crop is drawn at its actual smaller size. Unsafe margins or scale outline the main preview in red while allowing the manual decision to be saved. Candidate, motion direction, speed, and anchor edits append one sparse layer for exactly one locked beat range in the project JSON; older layers and unrelated beats remain intact. The review resolves current catalog paths on demand and checks artwork existence before preview/save. Its state/geometry and persistence are tested, but the revised Electron UI has not been interactively verified.
- M7 has a shared B-roll timeline compiler and an explicit Premiere XML export in the combined review. It appends direct still clips on a base artwork track and sparse override tracks above the unchanged phase-1 video/audio XML, using stored crop geometry for editable Basic Motion keyframes and hard cuts. The review displays an export track/clip preflight from this same compiler. Export checks the locked handoff, current catalog image/version/accepted revision, resolved file existence, and original source-media identities, and writes atomically. Unit tests cover phase-1 audio preservation, sparse stacking, cleared artwork, stale input rejection, and crop/keyframe mapping. Premiere import and visually correct motion endpoints are not yet verified.
...
- The Electron tagging UI smoke command currently fails in this automated Windows environment because its Chromium GPU subprocess cannot start, even with `--disable-gpu`; Node unit/integration tests pass, but these new UI controls have not been smoke-verified here.
